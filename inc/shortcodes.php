...
function digitalacademy_shortcode_businessCase( $atts ) {
	...
	if ( $businessCase->have_posts() ) {
		$out = '<div class="row clearfix container-list-block container-list-block-client text-center matchHeight-watch">';
		...
		$out .= '<div class="text-center"><a href="' . get_bloginfo( 'url' ) . '/etude-cas/" class="btn-gray m2020 mt0">Voir tous les business cases</a></div>';
...
function digitalacademy_shortcode_temoignages( $atts ) {
	...
	if ( $temoignages->have_posts() ) {
		...
		while ( $temoignages->have_posts() ) {
			...
			$img = get_field( 'visuel_carre', $temoignage->ID );
